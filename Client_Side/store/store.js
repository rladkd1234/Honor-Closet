...
export const store = new Vuex.Store({
    ...
    mutations: {
        ...
        setMyPurchaseList(state, payload){
    ...
    actions : {
        ...
        getPurchaseServer(context, {userId}){
            console.log('request success ');
            axios.get('http://202.31.202.253:5000/customer/'+userId+'/purchaselist')    //userId 를 id가 아닌 숫자로 알아야함.
            .then((response) => {
                this.commit('setMyPurchaseList', response.data); //작성
                console.log('getPurchaseServer request success ' + response.data);
              })
            .catch(response => {
                console.log('getPurchaseServer Request Fail');
                console.log(response)
            });
        },
        ...
        signUp(context, payload){
            ...
            return axios.post('http://202.31.202.253:5000/auth/join', payload)
            .then((response) =>{    //아이디 생성 성공
                console.log("success" + payload.E_mail + payload.isSolar + payload.Birth ) 
        ...
        setPurchaseServer(context, {userId, Itemid}){
            ...
            .then((response) => {
                //this.commit('setPurchaseList', response.data); 갱신해줄 필요 없음. 없나?
        ...
        setUnPurchaseServer(context, {userId, Itemid}){
            console.log(userId + Itemid);
            return axios.patch('http://202.31.202.253:5000/product/unpurchase/'+Itemid+'/', {customerName : userId})
            .then((response) => {
                this.dispatch('getPurchaseServer', {userId});// 구매 목록 갱신
                console.log('set Un Purchase list Request success : ' + response.data);
            })
            .catch(response => {
                console.log('Set un Purchase List Request Fail');
                console.log(response)
            });
        },
        ...
        setUnCartListServer(context, {userId, Itemid}){
            console.log(userId + Itemid);
            return axios.post('http://202.31.202.253:5000/customer/'+ userId  +'/uncart/', {Itemid})
            .then((response) => {
                this.dispatch('getCartServer', {userId});// 장바구니 갱신
        ...
        setUnLikeListServer(context, {userId, Itemid}){
            console.log(userId + Itemid);
            return axios.post('http://202.31.202.253:5000/customer/'+ userId  +'/unlike/', {Itemid})
            .then((response) => {
                this.dispatch('getLikeServer', {userId});// Like 목록 갱신
        ...
        setContentsServer(context, {Specification, Title, Contents, Writer, isHidden}){
            console.log(Specification + Title+ Writer);
            return axios.post('http://202.31.202.253:5000/board/', {Specification, Title, Contents, Writer, isHidden})
            .then((response) => {
                //갱신해줘야하나?ㅇㅇ
                this.dispatch('getQnaServer');
                console.log('set Contents Request success : ' + response.data);
            })
            .catch(response => {
                console.log('Set Contents Request Fail');
                console.log(response)
            });
        },
        delContentsServer(context, {id}){
            console.log(id+" is deleted");
            return axios.delete('http://202.31.202.253:5000/board/'+id+'/')
            .then((response) => {
                //갱신해줘야하나?ㅇㅇ
                this.dispatch('getQnaServer');
                console.log('delete Contents Request success : ' + response.data);
            })
            .catch(response => {
                console.log('Set Contents Request Fail');
                console.log(response)
        ...
        reviseContentsServer(context, {id, Specification, Title, Contents, Writer, isHidden}){
            console.log(Specification + Title+ Writer);
            return axios.patch('http://202.31.202.253:5000/board/'+id+'/', {Specification, Title, Contents, Writer, isHidden})
            .then((response) => {
                //갱신해줘야하나?ㅇㅇ
                this.dispatch('getQnaServer');
                console.log('patch Contents Request success : ' + response.data);
            })
            .catch(response => {
                console.log('patch Contents Request Fail');
                console.log(response)
            });
        },
    }
